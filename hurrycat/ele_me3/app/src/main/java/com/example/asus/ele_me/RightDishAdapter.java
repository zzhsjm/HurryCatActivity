...
public class RightDishAdapter extends RecyclerView.Adapter {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent,final int viewType) {
        if(viewType==MENU_TYPE){
            ...
            final Dish dish = getDishByPosition(viewType);
            view.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    //final int viewType;
                    final DishViewHolder holder=new DishViewHolder(view);
                    final android.app.AlertDialog.Builder builder=new android.app.AlertDialog.Builder(mContext);
                    LayoutInflater layoutInflater=LayoutInflater.from(mContext);
                    View AlertDialogView=layoutInflater.inflate(R.layout.alertdialog,null);
                    holder.alert1_tv=(TextView)AlertDialogView.findViewById(R.id.alert1);

                    holder.alert1_tv.setText(dish.getDishName());
                    builder.setView(AlertDialogView);
                    builder.create().show();
                    AlertDialogView.setTag(holder);
                }
            });
            return viewHolder;
        }
    ...
    private class DishViewHolder extends RecyclerView.ViewHolder{
        ...
        private LinearLayout right_dish_view;
        private ImageView right_dish_remove_iv;
        private ImageView right_dish_add_iv;
        private TextView right_dish_account_tv;
        private TextView alert1_tv;

        public DishViewHolder(View itemView) {
            super(itemView);
            right_dish_view=(LinearLayout)itemView.findViewById(R.id.right_dish_view);
